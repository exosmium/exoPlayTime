...
public class SockExchangeVelocity implements VelocityTieIn {
    ...
    public SockExchangeVelocity(ProxyServer server, Logger logger) {
        this.proxy = server;
        this.logger = logger;
    }
    ...
    private ConfigurationLoader<CommentedConfigurationNode> initializeConfigurationLoader() {
        final File configFilesLocation = Paths.get("plugins/exoplaytime").toFile();
        final Path configFileLocation = Paths.get(configFilesLocation + "/server-socket-config.yml");
        ...
        if (!Files.exists(configFileLocation)) {
            try {
                try (InputStream is = getClass().getClassLoader().getResourceAsStream("server-socket-config.yml")) {
