...
public class RunCmdChannelListener implements Consumer<ReceivedMessage>, Registerable
{
  ...
  public RunCmdChannelListener(
          SockExchangeSpigot plugin, SpigotPlugin spigotPlugin, BasicLogger basicLogger, SockExchangeApi api)
  {
  ...
  {
    ConsoleCommandSender consoleSender = spigotPlugin.getServer().getConsoleSender();

    for (String command : commands)
    {
      basicLogger.info(LOG_FORMAT, sourceServerName, command);

      spigotPlugin.getServer().dispatchCommand(consoleSender, command);
    }
  }
}
